 ...
 static class SimpleJson
    {
        ...
        static bool SerializeValue(IJsonSerializerStrategy jsonSerializerStrategy, object value, StringBuilder builder)
        {
            ...
            else
            {
                ...
                {
                    // MODIFIED to support nested dictionaries
                    // IDictionary<string, string> stringDictionary = value as IDictionary<string, string>; // Removed
                    var stringDictionary = Utils.TryConvertDictionary(value); // Added
 ...
 class PocoJsonSerializerStrategy : IJsonSerializerStrategy
    {
        ...
        public virtual object DeserializeObject(object value, Type type)
        {
            ...
            // MODIFIED START (added) https://github.com/facebook-csharp-sdk/simple-json/pull/64
            if (type.IsEnum)
            {
                ...
                if (objects != null)
                {
                    ...
                    if (ReflectionUtils.IsTypeDictionary(type))
                    {
                        ...
                        foreach (KeyValuePair<string, object> kvp in jsonObject) {
                            // MODIFIED START (added) Deserialize value as array instead of object
                            if (kvp.Value is JsonArray jsonArray) {
                                dict.Add(kvp.Key, DeserializeObject(kvp.Value, typeof(JsonArray)));
                            }
                            else { 
                                // MODIFIED END
        ...
        {
            // MODIFIED https://github.com/facebook-csharp-sdk/simple-json/pull/64
            // (disabled, may not always want this)
            //return p.ToString(); // Added
 ...
 class ReflectionUtils
        {
            ...
            public static object ToNullableType(object obj, Type nullableType)
            {
                ...
                // MODIFIED START Fix cast exception of long to enum
                var underlyingType = Nullable.GetUnderlyingType(nullableType);
                if (underlyingType.IsEnum && obj is long longValue) {
                    object parsed = Enum.ToObject(underlyingType, longValue);
                    return parsed;
                }
                else {
